...
final_metrics AS (
    SELECT
        ...
        SUM(CASE WHEN block_time >= now() - INTERVAL '6' MONTH THEN gas_cost_usd END) as gas_spent_usd_6_month,
        ...
        SUM(CASE WHEN block_time >= now() - INTERVAL '6' MONTH THEN gas_cost_native END) as gas_spent_native_6_month,
...
SELECT 
    ...
    COALESCE(gas_spent_usd_6_month, 0) as gas_spent_usd_6_month,
    ...
    COALESCE(gas_spent_native_6_month, 0) as gas_spent_native_6_month,
